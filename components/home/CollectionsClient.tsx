...
export type DynamicUniformItem = {
  id: string;
  img: string;
  title: string;
  desc: string;
...
export type DynamicCategory = {
  ...
  label: string;
  heading: string;
  items: DynamicUniformItem[];
};

export function ProductCard({ item, categoryLabel }: { item: DynamicUniformItem; categoryLabel: string }) {
  return (
    <div
    ...
    >
      ...
      <div className="flex flex-col flex-1 p-6">
        ...
        <p className="text-gray-400 text-sm leading-relaxed font-light flex-1 mb-6">
          {item.desc || 'Premium quality uniform designed for comfort and durability.'}
        </p>
...
export default function CollectionsClient({ categories }: { categories: DynamicCategory[] }) {
  const [active, setActive] = useState(categories[0]?.id);
  const current = categories.find((c) => c.id === active) ?? categories[0];

  if (!categories || categories.length === 0) {
    return (
      <section id="collections" className="w-full bg-[#1C1C1C] py-20 text-center">
      ...
      <div className="max-w-[1600px] mx-auto px-4 md:px-16 pt-14 pb-20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {current?.items.map((item) => (
            <ProductCard key={item.id} item={item} categoryLabel={current.label} />
          ))}
          {current?.items.length === 0 && (
            <div className="col-span-full text-center text-gray-500 py-10">
              No items uploaded for this category yet.
            </div>
          )}
        </div>
      </div>
